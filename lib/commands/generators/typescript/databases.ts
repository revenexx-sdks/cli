...
${
  ...
  private generateTypesFile(config: ConfigType, sdkDep: string): string {
  ...
  async generate(
    ...
    const sdkDep =
      options?.sdkImportSource ?? getSdkDependency();
    ...
    if (!hasEntities) {
      ...
      return {
        ...
        constantsContent: this.generateConstantsFile(config, sdkDep),
      };
    }

    return {
      dbContent: this.generateDatabasesFile(config, importExt, sdkDep),
      typesContent: this.generateTypesFile(config, sdkDep),
      indexContent: this.generateIndexFile(importExt),
      constantsContent: this.generateConstantsFile(config, sdkDep),
    };
  }
