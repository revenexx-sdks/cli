      ...
      columns.forEach((column, columnIndex) => {
        if (column.type === "relationship" && column.relatedTable) {
          ...
          if (relatedDatabaseId && relatedDatabaseId !== table.databaseId) {
            ctx.addIssue({
              code: z.ZodIssueCode.custom,
              message: `Invalid cross-database relationship: Table '${table.name}' (database: '${table.databaseId}') has relationship '${column.key}' pointing to table '${column.relatedTable}' which is in database '${relatedDatabaseId}'. Revenexx only supports relationships within the same database.`,
              path: [
                "tables",
                tableIndex,
                "columns",
      ...
      attributes.forEach((attribute, attributeIndex) => {
        if (attribute.type === "relationship" && attribute.relatedCollection) {
          ...
          if (
            ...
            ctx.addIssue({
              code: z.ZodIssueCode.custom,
              message: `Invalid cross-database relationship: Collection '${collection.name}' (database: '${collection.databaseId}') has relationship '${attribute.key}' pointing to collection '${attribute.relatedCollection}' which is in database '${relatedDatabaseId}'. Revenexx only supports relationships within the same database.`,
              path: [
                "collections",
                collectionIndex,
                "attributes",
